...
def meetup_day(year, month, name_of_day, day_of_month):
    ...
    # adding arguments: 
    #name_of_day(M,Tu,W,Th,Fr,Sa,Su) & day_of_month(1st,2nd,3rd,4th,teenth,last) 
    ...
    #returns cal iterator(tuple) for specified year, month (date.day#, wkday#)
    cal = list(cal_inst.itermonthdays2(year, month))
    
    #append a new list with all of x weekday values (all Mondays, Tuesdays, etc)
